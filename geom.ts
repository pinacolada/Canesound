...
class Matrix {
    ...
    toString(): string { return this.m3.toString(); }

    get a(): number { return this.m3.a; } set a(value: number) { this.m3.a = value; }
    get b(): number { return this.m3.b; } set b(value: number) { this.m3.b = value; }
    get c(): number { return this.m3.c; } set c(value: number) { this.m3.c = value; }
    get d(): number { return this.m3.d; } set d(value: number) { this.m3.d = value; }
    get e(): number { return this.m3.e; } set e(value: number) { this.m3.e = value; }
    get f(): number { return this.m3.f; } set f(value: number) { this.m3.f = value; }

    flipX() { this.m3.flipX(); }
    flipY() { this.m3.flipY(); }
    inverse() { this.m3.inverse(); }
    multiply(secondMatrix: Matrix) { this.m3.multiply(secondMatrix.m3); }
    rotate(angle: number) { this.m3.rotate(angle); }
    rotateFromVector(x: number, y: number) { this.m3.rotateFromVector(x, y); };
    scale(scaleFactor: number) { this.m3.scale(scaleFactor); }
    scaleNonUniform(scaleFactorX: number, scaleFactorY: number) { this.m3.scaleNonUniform(scaleFactorX, scaleFactorY); }
    skewX(angle: number) { this.m3.skewX(angle); }
    skewY(angle: number) { this.m3.skewY(angle); }
    translate(x: number, y: number) { this.m3.translate(x, y); }
}
class Matrix3D {
    m4: WebKitCSSMatrix;
    constructor(m?: WebKitCSSMatrix | null) {
        this.m4 = new WebKitCSSMatrix();
        if (m != null) this.getValuesFrom(m);
    }

    toString(): string { return this.m4.toString(); }

    getValuesFrom(m: WebKitCSSMatrix) {
        this.m4.m11 = m.m11; this.m4.m21 = m.m21; this.m4.m31 = m.m31; this.m4.m41 = m.m41;
        this.m4.m12 = m.m12; this.m4.m22 = m.m21; this.m4.m32 = m.m32; this.m4.m42 = m.m42;
        this.m4.m13 = m.m13; this.m4.m23 = m.m23; this.m4.m33 = m.m33; this.m4.m43 = m.m43;
        this.m4.m14 = m.m14; this.m4.m24 = m.m24; this.m4.m34 = m.m34; this.m4.m44 = m.m44;
    }
    multiply(alt: Matrix3D): Matrix3D {
        return new Matrix3D(this.m4.multiply(alt.m4));
    }
    preMultiply(alt: Matrix3D): Matrix3D {
    ...
    scale(x: number, y: number): Matrix3D {
        return new Matrix3D(this.m4.scale(x, y));
    }
    scale3d(x: number, y: number, z: number): Matrix3D {
        return new Matrix3D(this.m4.scale(x, y, z));
    }
    rotate(angleX: number, angleY?: number | undefined, angleZ?: number | undefined): Matrix3D {
        return new Matrix3D(this.m4.rotate(angleX, angleY, angleZ));
    }
    rotateFromVector(x: number, y: number, z: number, angle: number): Matrix3D {
        return new Matrix3D(this.m4.rotateAxisAngle(x, y, z, angle));
    }
    skewX(angle: number): Matrix3D {
        return new Matrix3D(this.m4.skewX(angle));
    }
    skewY(angle: number): Matrix3D {
        return new Matrix3D(this.m4.skewY(angle));
    }
    inverse(): Matrix3D {
        return new Matrix3D(this.m4.inverse());
    }
    setMatrixValue(values: string) {
